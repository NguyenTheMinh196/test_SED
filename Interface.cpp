            ...
            if (update_item == NULL) {
                ...
                if(kind == 1){
                    ...
                    if(newRentalType == 1){ //change type to DVD
                        ...
                        cout<<"We will be deleting the genre section of the item to change this item rental type to Game";
                        vector<Item*> items;
                    ...
                    cout << "Reminder: If you change the status of the item, the number of copy will change accordingly!";
...
void Interface::menu2() {
    ...
    if(subChoice == 1){
        ...
        int numOfRental = atoi(numOfRentals.c_str());
        ...
        if(customerkind == 1){
            customerType == "Guest";
            if(numOfRental > 2){
                cout << "Guest can not borrow more than 2 items! Please choose again the number of rental item this customer loaned: ";
                getline(cin, numOfRentals);
                bool checkRental = vl->checkInt(numOfRentals);
                int numOfRental = atoi(numOfRentals.c_str());
            }
        ...
        int num_of_vid = 0;
        if (checkRental) {
            while(listItem.size() < numOfRental) {
                this->rs->printAllItemSortedID(1);
                cout<<"Please enter the ID of the item that the customer want to loan: ";
                getline(cin, items);
                vl->checkIdItem(items, listItem); //assuming that they do not loan 2 of the same type
                ...
                if(new_item->getNumOfCopy() == 0){
                    ...
                    if(customerType == "Guest") {
                        if (new_item->getLoanType() == "Video" && new_item->getLoanType() == "2-day"){
                            cout<<"Guest can not loan a video for two day. Please upgrade your membership!";
                        }
                            ...
                            numOfRental += numOfRental;
                            listItem.push_back(new_item->getID());
                        }
                        else if(num_of_vid > 2){
                            cout<<"The maximum amount of video that the guest can loan is 2! Please loan another item";
                        }
                    }
                    else {
                        new_item->setNumOfCopy(new_item->getNumOfCopy() - 1);
        ...
        if (checkId && checkRental && vl->checkCustomer(id, CUSs, name, address, phone, numOfRentals, customerType, listItem)) {
